...
const ModalInfo = ({ showModal, handleClose }) => {
  ...
  return (
    <Modal show={showModal} onHide={handleClose} backdrop="static" keyboard={false} size="xl" aria-labelledby="contained-modal-title-vcenter"
    centered>
      <Modal.Header className={styles.modalPadding} closeButton>
        <Modal.Title className={styles.headingInfo2}>Säännöt Sortti-peräkärryn vuokraukseen ja käyttöön</Modal.Title>
      </Modal.Header>
      <Modal.Body className={styles.modalPadding}>
        <RentInfoBoxList items={items} />
        <p>Perimme jätteiden vastaanotosta Sortti-asemien hinnaston mukaisen maksun. Vuokran voi maksaa INFOssa kuorman tuonnin yhteydessä.</p>
        <Form.Check
          type="checkbox"
      ...
      <Modal.Footer className={styles.modalFooter}>
        <Button className={styles.rentButton} variant="primary" onClick={handleRentClick} disabled={!isChecked}>
          Vuokraamaan
        </Button>
      </Modal.Footer>
    </Modal>
